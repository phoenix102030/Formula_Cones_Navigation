...
PyArrayFloatPair getTrackCones(const float propagation_dist, 
    ...
    for (int i = 0; i < cones.size(); i+=2) {
        float x = center_points[i].x;
        float y = center_points[i].y;
        Cone left_cone(cones[i].x, cones[i].y);
        Cone right_cone(cones[i+1].x, cones[i+1].y);
        Waypoint waypoint(x, y, left_cone, right_cone);
        waypoints.push_back(waypoint);
    }

    addSpeedProfile(waypoints);



    return std::make_pair(cones_arr, track.getPropagationAngle(propagation_dist), waypoints);
}
