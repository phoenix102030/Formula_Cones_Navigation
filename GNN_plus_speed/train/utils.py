...
def prepare_data(num_tracks, prop_dist, detection_prob, max_false_positives, max_prop_angle, min_perception_range, max_perception_range):
    ...
    for _ in tqdm(range(num_tracks)):
        cones, angle, waypoints = fsg.get_track_cones(propagation_dist=prop_dist,                                        
        ...
        speed = [waypoint.speed for waypoint in waypoints]
        if len(cones) < 3:
            continue
        ...
        X.append(graph)
        Y.append([angle / max_prop_angle, speed])
    return X, Y

def data_align(angle)